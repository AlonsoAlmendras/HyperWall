...
func _estimate_body_forward_dir() -> Vector3:
	...
	if camera_elevation > 0.75:
		# User is looking up
		var slide_vector = -camera_basis.y.slide(up_player)
		if slide_vector.length() > 0.0001:
			forward = slide_vector.normalized()
		else:
			forward = Vector3.FORWARD  # Usar un valor predeterminado
		...
		var slide_vector = camera_forward.slide(up_player)
		if slide_vector.length() > 0.0001:
			forward = slide_vector.normalized()
		else:
			forward = Vector3.FORWARD  # Usar un valor predeterminado
	...
	if (left_hand_node and left_hand_node.get_is_active()
		...
		var slide_tangent = tangent.slide(up_player)
		if slide_tangent.length() > 0.0001:
			tangent = slide_tangent.normalized()
		else:
			tangent = Vector3.RIGHT  # Usar un valor predeterminado

		var hands_forward = up_player.cross(tangent)
		if hands_forward.length() > 0.0001:
			hands_forward = hands_forward.normalized()
		else:
			hands_forward = Vector3.FORWARD  # Usar un valor predeterminado

		# Rotate our forward towards our hand direction but not more than 60 degrees
		var dot = forward.dot(hands_forward)
		var cross = forward.cross(hands_forward)
		if cross.length() > 0.0001:
			cross = cross.normalized()
		else:
			cross = Vector3.UP  # Usar un valor predeterminado
